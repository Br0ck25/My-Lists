...
- **Next Up channel.** One button, and no picks to make: the channel is re-derived from your Continue Watching on every request, so pressing play always serves the next unwatched episode across everything you have on the go.
...
- **Spotlight channels.** Search an actor, director or creator next to Shows and Movies in the builder, and their best films and TV work compile into one channel, in career order or best-first.
...
- **Share links and the Explore Channels directory.** **Share** on any channel copies a link that rebuilds it anywhere -- every pick, its play order and its broadcast schedule. Publishing (with a Creator Profile) lists it in **Explore Channels**, where anyone can preview and add it in one click.
