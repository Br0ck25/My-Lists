...
- **A season says "watched" only when its episodes are.** Every season button on a show read "✓ Mark Season Unwatched" with nothing watched in any of them. `isSeasonFullyWatched` began by consulting `_fullyWatchedShowIds` and returning true if the show was in it — but that set means "Continue Watching has nothing left to offer for this show", and dismissing a show from Continue Watching puts it there deliberately. One dismissal flipped all 24 seasons. The season now answers from the episodes this browser can actually prove, and the cached episode list is keyed per show rather than per season number (one show's 10-episode season 3 was the yardstick for another's 22-episode one).
- **A failed season lookup no longer means "the show has ended".** Continue Watching asks TMDB for season N+1 to learn whether a show is finished, and `/api/season` answered the same 404 for "no such season" and for a rate limit or a 5xx — so a network hiccup filed the show under fully-watched. The route now reports whether the season is really absent, and an unknown answer leaves the badge alone, which is what the code's own comment always promised.
